...
static class FrequencyAnalysisTask
{
    public static Dictionary<string, string[]> GetMostFrequentNextWords(List<List<string>> text)
    {
        ...
        foreach (var dictionaryForOneElem in dictionaryOfAllVar)
        {
            ...
            var arrayOfMAx = new List<string>();
            for (var i = 0; i < 4; i++)
            {
                if (dictionaryWordPopularity.Count == 0)
                {
                ...
                var maxString = Smth(dictionaryWordPopularity);
                arrayOfMAx.Add(maxString); 
                dictionaryWordPopularity.Remove(maxString); 
            }
            result.Add(value, arrayOfMAx.ToArray());
        }
        return result;
    }

    public static string Smth(Dictionary<string, int> dictionaryWordPopularity)
    {
        ...
        return CompareStringAndReturnMax(frequentKeys);
